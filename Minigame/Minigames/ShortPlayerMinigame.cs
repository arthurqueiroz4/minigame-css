...
public class ShortPlayerMinigame : IMinigame
{
    public ShortPlayerMinigame(BasePlugin plugin)
    {
        Plugin = plugin;
    }
    public BasePlugin Plugin { get; }
    public string Name => "Short Players";

    private List<CCSPlayerPawn> playersWithChangedModelSize = new();
    private float shortSize = 0.7f;
    ...
    private void ChangePlayerSize(CCSPlayerController? player)
    {
        ...
        if (playerSceneNode == null)
            return;
        playerSceneNode.GetSkeletonInstance().Scale = shortSize;
        playerPawn.AcceptInput("SetScale", null, null, shortSize.ToString());
        Server.NextFrame(() =>
        {
